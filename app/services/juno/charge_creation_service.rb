  ...
  class ChargeCreationService
    ...
    def create_charges
      ...
      Juno::Charge.transaction do
        charges.each_with_index { |charge, index| create_charge(charge, index + 1) }
      end
    ...
    def create_credit_card_payment
      credit_card_payment = JunoApi::CreditCardPayment.new.create!(@order)
      Juno::CreditCardPayment.transaction do
        credit_card_payment.each { |payment| create_charge_payment(payment) }
      end
